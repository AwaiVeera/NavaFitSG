...
let firebaseApp;
let firebaseAuth;
let emailMode = (modeParam === 'signup' || modeParam === 'sign-up') ? 'signUp' : 'signIn';
...
function applyEmailModeUi() {
    ...
    if (toggle) {
        toggle.textContent = emailMode === 'signIn' ?
            'Need an account? Switch to Sign Up' :
            'Already have an account? Sign In';
    }
    const label = document.querySelector('[data-email-mode-label]');
    if (label) {
        label.textContent = emailMode === 'signIn' ?
            'Sign In with Email' :
            'Sign Up with Email';
    }
}
